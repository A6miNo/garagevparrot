    ...
    if (isset($_SESSION['user']) && $role == "Administrateur") {
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                echo '<td>' . $row["jour"] . '</td>';
                echo '<td><input class="form-control" type="time" name="heure_ouverture_am[]" value="' . $row["heure_ouverture_am"] . '"></td>';
                echo '<td><input class="form-control" type="time" name="closeam[]" value="' . $row["heure_fermeture_am"] . '"></td>';
                echo '<td><input class="form-control" type="time" name="openpm[]" value="' . $row["heure_ouverture_pm"] . '"></td>';
                echo '<td><input class="form-control" type="time" name="closepm[]" value="' . $row["heure_fermeture_pm"] . '"></td>';
                echo '<td>';
